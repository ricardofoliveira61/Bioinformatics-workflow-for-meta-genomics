...
tax_class = "C:\\Users\\ricar\\Desktop\\Teste\\assignment_results\\v4\\v4_classification_results.xlsx"
...
if ("confidence" %in% colnames(tax)){
  taxonomic = tax[, !(colnames(tax) == "confidence")]
}
tax = as.matrix(tax)
...
phylo_bac = filter_taxa(phylo_bac_all, function(x) sum(x > 10) > (0.04*length(x)), TRUE)
